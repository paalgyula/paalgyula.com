...
const DraftEditorProvider: FC<PropsWithChildren> = ({ children }) => {
    ...
    return (
        <DraftEditorContext.Provider value={{
            editorState,
            setEditorState
        }}>{children}</DraftEditorContext.Provider>
    )
}
...
export const useEditorState = (): DraftEditorProps => {
    ...
    if (!ctx) {
        throw new Error('useEditorState must be wrapped with DraftEdirtorProvider component')
    }
