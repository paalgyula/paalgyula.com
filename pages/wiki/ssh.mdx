...
In addition to remote terminal access provided by the main ssh binary, the SSH suite of programs has grown to include other tools such as _[SCP](#scp-secure-copy)_ (Secure Copy Program) and _[SFTP](#sftp-secure-file-transfer-protocol)_ (Secure File Transfer Protocol).
...
export const Layout = ({ meta, children }) => <>{children}</>;
